...
resource "vault_policy" "secrets" {
  ...
  policy = <<-EOT
    ...
    path "${var.applications_mount_path}/data/outline/sgfdevs/*" {
      capabilities = ["read"]
    }
  EOT
}
...
resource "vault_kubernetes_auth_backend_role" "secrets" {
  ...
  role_name                        = "outline-sgfdevs-secrets"
  bound_service_account_names      = ["outline-secrets"]
  bound_service_account_namespaces = ["outline-sgfdevs"]
  audience                         = "vault"
