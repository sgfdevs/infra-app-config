...
resource "aws_s3_bucket_cors_configuration" "assets" {
  ...
  cors_rule {
    ...
    allowed_origins = ["https://docs.opensgf.org"]
    expose_headers  = ["ETag"]
    max_age_seconds = 300
  }
}
...
resource "aws_iam_role" "outline" {
  name                 = "sgfdevs-k3s-outline-opensgf"
  path                 = "/sgfdevs-k3s/"
  description          = "Allow Outline opensgf to manage its private S3 assets"
    ...
    Statement = [{
      ...
      Condition = {
        StringEquals = {
          ...
          "${local.k3s_oidc_issuer}:sub" = "system:serviceaccount:outline-opensgf:outline-secrets"
        }
      }
    }]
  })
  ...
  tags = {
    KubernetesNamespace      = "outline-opensgf"
